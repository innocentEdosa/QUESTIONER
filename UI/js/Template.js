...
function happening(date) {
  ...
  let happeningDate = new Date(date);
  const day = happeningDate.getDay();
  const month = monthNames[happeningDate.getMonth()];
  const year = happeningDate.getFullYear();
  happeningDate = `${day} / ${month} / ${year}`;
